  ...
  methods: {
    subscribeToUpdates(tickerName) {
      setInterval(async () => {
        ...
        // currentTicker.price =  data.USD > 1 ? data.USD.toFixed(3) : data.USD.toPrecision(2);
    ...
    add() {
      if (this.tickers.filter(e => e.name === this.ticker.toUpperCase()).length) {
        this.ticker = "";
        this.isDubleTicker = true;
        return;
      }
